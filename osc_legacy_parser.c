...
#define yylval          osc_legacy_parser_lval
#define yychar          osc_legacy_parser_char
#define yydebug         osc_legacy_parser_debug
#define yynerrs         osc_legacy_parser_nerrs
#define yylloc          osc_legacy_parser_lloc
...
int osc_legacy_parser_parse (void *YYPARSE_PARAM);
#else
int osc_legacy_parser_parse ();
#endif
#else /* ! YYPARSE_PARAM */
#if defined __STDC__ || defined __cplusplus
...
{
      19,     0,    -1,    -1,    19,    21,    -1,    11,    -1,    12,
      -1,     3,    -1,     4,    -1,     7,    -1,     8,    -1,     9,
      -1,    10,    -1,    13,    -1,    14,    -1,    20,    11,    -1,
      20,    12,    -1,    20,     3,    -1,    20,     4,    -1,    20,
       7,    -1,    20,     8,    -1,    20,     9,    -1,    20,    10,
      -1,    20,    13,    -1,    20,    14,    -1,    15,    19,    16,
      -1,    15,    17,    19,    16,    -1,    20,    15,    19,    16,
      -1,    20,    15,    17,    19,    16,    -1,    12,    20,    17,
      -1,    12,    17,    -1
};
...
static const char *const yytname[] =
{
  ...
  "$accept", "bundle", "arglist", "msg", YY_NULL
};
#endif
...
#else
static void
yy_symbol_print (yyoutput, yytype, yyvaluep, yylocationp, bndl, msg, scanner, buflen, buf)
    FILE *yyoutput;
    int yytype;
    YYSTYPE const * const yyvaluep;
    YYLTYPE const * const yylocationp;
...
`------------------------------------------------*/
     ...
     || defined __cplusplus || defined _MSC_VER)
static void
yy_reduce_print (YYSTYPE *yyvsp, YYLTYPE *yylsp, int yyrule, t_osc_parser_bndl_list **bndl, t_osc_msg_u **msg, void *scanner, long *buflen, char **buf)
#else
static void
yy_reduce_print (yyvsp, yylsp, yyrule, bndl, msg, scanner, buflen, buf)
    YYSTYPE *yyvsp;
    YYLTYPE *yylsp;
    int yyrule;
    t_osc_parser_bndl_list **bndl;
...
do {					\
  if (yydebug)				\
    yy_reduce_print (yyvsp, yylsp, Rule, bndl, msg, scanner, buflen, buf); \
} while (YYID (0))
...
#else
static void
yydestruct (yymsg, yytype, yyvaluep, yylocationp, bndl, msg, scanner, buflen, buf)
    const char *yymsg;
    int yytype;
    YYSTYPE *yyvaluep;
    YYLTYPE *yylocationp;
...
#else
int
yyparse (bndl, msg, scanner, buflen, buf)
    t_osc_parser_bndl_list **bndl;
    t_osc_msg_u **msg;
    void *scanner;
    long *buflen;
...
#line 273 "osc_legacy_parser.y"
    {
		...
		m->next = *msg;
		*msg = m;
		osc_message_u_appendTimetag(*msg, (yyvsp[(1) - (1)].t));
	  }
    break;
...
#line 317 "osc_legacy_parser.y"
    {
		PP("add OSCTIMETAG to MSG %p := %llu\n", *msg, (yyvsp[(2) - (2)].t));
		osc_message_u_appendTimetag(*msg, (yyvsp[(2) - (2)].t));
 	}
    break;

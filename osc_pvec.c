...
t_osc_pvec_ud *osc_pvec_ud_alloc(t_osc_region r, void *userdata);
...
{
	OSC_PVEC_ENTER;
	t_osc_pvec_ud *ud = osc_region_getBytes(r, sizeof(t_osc_pvec_ud));
	if(ud){
		ud->data = userdata;
		ud->refcount = 1;
	}
...
t_osc_pvec_node *osc_pvec_node_allocWithArray(t_osc_region r, int length, void **array);
t_osc_pvec_node *osc_pvec_node_clone(t_osc_region r, t_osc_pvec_node *n);
t_osc_pvec_node *osc_pvec_node_copy(t_osc_pvec_node *n);
void **osc_pvec_node_getArray(t_osc_pvec_node *node);

t_osc_pvec_node *osc_pvec_node_alloc(t_osc_region r)
{
	OSC_PVEC_ENTER;
	t_osc_pvec_node *n = osc_pvec_node_allocWithArray(r, 0, NULL);
...
{
	OSC_PVEC_ENTER
	t_osc_pvec_node *node = osc_region_getBytes(r, sizeof(t_osc_pvec_node));
...
t_osc_pvec_node *osc_pvec_node_clone(t_osc_region r, t_osc_pvec_node *n)
{
	...
	t_osc_pvec_node *clone = osc_pvec_node_allocWithArray(r, OSC_PVEC_NBRANCHES, osc_pvec_node_getArray(copy));
	OSC_PVEC_DECRC(copy);
...
t_osc_pvec_node *osc_pvec_doAssoc(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *node, int i, void *val);
t_osc_pvec *osc_pvec_cons(t_osc_region r, t_osc_pvec *pvec, void *val);
t_osc_pvec_node *osc_pvec_pushTail(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *parent, t_osc_pvec_node *tailnode);
t_osc_pvec_node *osc_pvec_newPath(t_osc_region r, int level, t_osc_pvec_node *node);
t_osc_pvec_node *osc_pvec_popTail(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *node);
...
void **osc_pvec_cloneTail(t_osc_region r, int len, void **array);
void **osc_pvec_copyTail(int len, void **array);

//t_osc_pvec_node *osc_pvec_newPath_m(int level, t_osc_pvec_node *node);
t_osc_pvec_node *osc_pvec_doAssoc_m(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *node, int i, void *val);
t_osc_pvec *osc_pvec_cons_m(t_osc_region r, t_osc_pvec *pvec, void *val);
t_osc_pvec_node *osc_pvec_pushTail_m(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *parent, t_osc_pvec_node *tailnode);
t_osc_pvec_node *osc_pvec_newPath_m(t_osc_region r, int level, t_osc_pvec_node *node);

t_osc_pvec *_osc_pvec_alloc(t_osc_region r, int count, int shift, t_osc_pvec_node *root, int taillength, void **tail, void (*freefn)(void *))
{
	OSC_PVEC_ENTER
	t_osc_pvec *pvec = osc_region_getBytes(r, sizeof(t_osc_pvec));
	pvec->count = count;
	pvec->shift = shift;
	pvec->root = root;
...
t_osc_pvec *osc_pvec_alloc(t_osc_region r, void (*freefn)(void *))
{
	OSC_PVEC_ENTER
	t_osc_pvec *pvec = _osc_pvec_alloc(r, 0, OSC_PVEC_NBITS, osc_pvec_node_alloc(r), 0, NULL, freefn);
	OSC_PVEC_RETURN_OBJ(pvec, osc_pvec_printRefCount);
}
...
void **osc_pvec_cloneTail(t_osc_region r, int len, void **tail)
{
	OSC_PVEC_ENTER
	// just increments the refcounts
	void **copy = osc_pvec_copyTail(len, tail);
	// now make a copy and return
	void **newtail = (void **)osc_region_getBytes(r, len * sizeof(void *));
...
t_osc_pvec *osc_pvec_assocN(t_osc_region r, t_osc_pvec *pvec, int i, void *val)
{
	OSC_PVEC_ENTER
	...
	void (*freefn)(void *) = osc_pvec_getFreeFn(pvec);
	{
		t_osc_pvec_ud *ud = osc_pvec_ud_alloc(r, val);
		if(i >= 0 && i < osc_pvec_length(pvec)){
			P("i(%d) is less than count(%d)", i, count);
			if(i >= osc_pvec_tailOffset(pvec)){
				P("%d is in the tail", i);
				
				void **newtail = (void **)osc_region_getBytes(r, taillength * sizeof(void *));
				...
				t_osc_pvec *ret = _osc_pvec_alloc(r, count, shift, osc_pvec_node_copy(root), taillength, newtail, freefn);
				OSC_PVEC_RETURN_OBJ(ret);
			}
			P("i(%d) is not in the tail", i);
			t_osc_pvec *pv = _osc_pvec_alloc(r, count, shift, osc_pvec_doAssoc(r, pvec, shift, osc_pvec_node_copy(root), i, ud), taillength, tail, freefn);
			OSC_PVEC_RETURN_OBJ(pv);
		}
		if(i == osc_pvec_length(pvec)){
			P("i(%d) == count(%d)", i, osc_pvec_length(pvec));
			t_osc_pvec *pv = osc_pvec_cons(r, pvec, ud);
			OSC_PVEC_RETURN_OBJ(pv)
		}
...
t_osc_pvec *osc_pvec_assocN_m(t_osc_region r, t_osc_pvec *pvec, int i, void *val)
{
	OSC_PVEC_ENTER;
	if(OSC_PVEC_GETRC(pvec) != 1){
		t_osc_pvec *pv = osc_pvec_assocN(r, pvec, i, val);
	...
	void (*freefn)(void *) = osc_pvec_getFreeFn(pvec);
	{
		...
		if(i >= 0 && i < osc_pvec_length(pvec)){
			...
			P("i(%d) is not in the tail", i);
			//t_osc_pvec *pv = _osc_pvec_alloc(count, shift, osc_pvec_doAssoc_m(r, pvec, shift, osc_pvec_node_copy(root), i, ud), taillength, tail, freefn);
			//OSC_PVEC_RETURN_OBJ(pv);
			osc_pvec_setRoot(pvec, osc_pvec_doAssoc_m(r, pvec, shift, root, i, ud));
			OSC_PVEC_RETURN_OBJ(pvec);
		}
		if(i == osc_pvec_length(pvec)){
			P("i(%d) == count(%d)", i, osc_pvec_length(pvec));
			t_osc_pvec *pv = osc_pvec_cons_m(r, pvec, ud);
			OSC_PVEC_RETURN_OBJ(pv);
		}
		P("i(%d) is out of bounds (%d)", i, osc_pvec_length(pvec));
	}
...
{
	OSC_PVEC_ENTER
	void **clone = (void **)osc_region_getBytes(r, OSC_PVEC_NBRANCHES * sizeof(void *));
	memcpy(clone, osc_pvec_node_getArray(node), OSC_PVEC_NBRANCHES * sizeof(void *));
	t_osc_pvec_node *ret = osc_pvec_node_allocWithArray(r, OSC_PVEC_NBRANCHES, clone);
	P("level = %d", level);
	if(level == 0){
		...
		osc_pvec_node_getArray(ret)[subidx] = osc_pvec_doAssoc(r, pvec, level - OSC_PVEC_NBITS, (t_osc_pvec_node *)osc_pvec_node_getArray(node)[subidx], i, val);
	}
	OSC_PVEC_RETURN_OBJ(ret);
}
...
t_osc_pvec_node *osc_pvec_doAssoc_m(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *node, int i, void *val)
{
	OSC_PVEC_ENTER;
	if(OSC_PVEC_GETRC(pvec) != 1){
		t_osc_pvec_node *n = osc_pvec_doAssoc(r, pvec, level, node, i, val);
	...
	if(level == 0){
		...
		int subidx = (i >> level) & OSC_PVEC_MASK;
		osc_pvec_node_getArray(ret)[subidx] = osc_pvec_doAssoc_m(r, pvec, level - OSC_PVEC_NBITS, (t_osc_pvec_node *)osc_pvec_node_getArray(node)[subidx], i, val);
	}
	OSC_PVEC_RETURN_OBJ(ret);
}
...
t_osc_pvec *osc_pvec_cons(t_osc_region r, t_osc_pvec *pvec, void *val)
{
	...
	if(count - osc_pvec_tailOffset(pvec) < OSC_PVEC_NBRANCHES){
		...
		void **newtail = (void **)osc_region_getBytes(r, (taillength + 1) * sizeof(void *));
		if(tail){
		...
		OSC_PVEC_INCRC(root);
	        t_osc_pvec *pv = _osc_pvec_alloc(r, count + 1, shift, root, taillength + 1, newtail, freefn);
		OSC_PVEC_RETURN_OBJ(pv);
	}
	P("no room in the tail");
	t_osc_pvec_node *newroot;
	void **tailclone = osc_pvec_cloneTail(r, taillength, tail);
	t_osc_pvec_node *tailnode = osc_pvec_node_allocWithArray(r, taillength, tailclone);
	//////////////////////////////////////////////////
	...
	if((count >> OSC_PVEC_NBITS) > (1 << shift)){
		...
		newroot = osc_pvec_node_alloc(r);
		OSC_PVEC_INCRC(root);
		osc_pvec_node_getArray(newroot)[0] = root;//osc_pvec_node_copy(root);
		osc_pvec_node_getArray(newroot)[1] = osc_pvec_newPath(r, shift, tailnode);
		...
		newroot = osc_pvec_pushTail(r, pvec, shift, root, tailnode);
		OSC_PVEC_INCRC(newroot);
	}
	void **array = (void **)osc_region_getBytes(r, OSC_PVEC_NBRANCHES * sizeof(void *));
	memset(array, 0, OSC_PVEC_NBRANCHES * sizeof(void *));
	array[0] = val;
	t_osc_pvec *pv = _osc_pvec_alloc(r, count + 1, newshift, newroot, 1, array, freefn);
	OSC_PVEC_RETURN_OBJ(pv);
}
...
t_osc_pvec *osc_pvec_cons_m(t_osc_region r, t_osc_pvec *pvec, void *val)
{
	...
	if(OSC_PVEC_GETRC(pvec) != 1){
		t_osc_pvec *pv = osc_pvec_cons(r, pvec, val);
		osc_pvec_release(pvec);
		OSC_PVEC_RETURN_OBJ(pv);
	}
	int count = osc_pvec_length(pvec);
	...
	if(count - osc_pvec_tailOffset(pvec) < OSC_PVEC_NBRANCHES){
		...
		void **newtail = tail;
		if(!newtail){
			//newtail = (void **)osc_region_getBytes(r, (taillength + 1) * sizeof(void *));
			newtail = (void **)osc_region_getBytes(r, OSC_PVEC_NBRANCHES * sizeof(void *));
			memset(newtail, 0, (taillength + 1) * sizeof(void *));
	...
	t_osc_pvec_node *tailnode = osc_pvec_node_allocWithArray(r, taillength, tail);
	...
	if((count >> OSC_PVEC_NBITS) > (1 << shift)){
		...
		osc_pvec_node_getArray(newroot)[1] = osc_pvec_newPath_m(r, shift, tailnode);
		newshift += OSC_PVEC_NBITS;
	}else{
		P("room at the current level: push tail");
		//newroot = osc_pvec_node_copy(osc_pvec_pushTail(pvec, shift, root, tailnode));
		newroot = osc_pvec_pushTail_m(r, pvec, shift, root, tailnode);
		//OSC_PVEC_INCRC(newroot);
	}
	void **array = tail;
	if(!array){
		array = (void **)osc_region_getBytes(r, OSC_PVEC_NBRANCHES * sizeof(void *));
	}
	memset(array, 0, OSC_PVEC_NBRANCHES * sizeof(void *));
	array[0] = val;
...
t_osc_pvec_node *osc_pvec_pushTail(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *parent, t_osc_pvec_node *tailnode)
{
	...
	//void *parent_clone = (void **)osc_region_getBytes(r, OSC_PVEC_NBRANCHES * sizeof(void *));
	...
	if(level == OSC_PVEC_NBITS){
		...
		if(child == NULL){
			P("no child");
			nodeToInsert = osc_pvec_newPath(r, level - OSC_PVEC_NBITS, tailnode);
		}else{
			P("have child");
			nodeToInsert = osc_pvec_pushTail(r, pvec, level - OSC_PVEC_NBITS, child, tailnode);
		}
	}
...
t_osc_pvec_node *osc_pvec_pushTail_m(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *parent, t_osc_pvec_node *tailnode)
{
	OSC_PVEC_ENTER;
	if(OSC_PVEC_GETRC(pvec) != 1){
		t_osc_pvec_node *n = osc_pvec_pushTail(r, pvec, level, parent, tailnode);
		OSC_PVEC_RETURN_OBJ(n);
	}
	int subidx = ((osc_pvec_length(pvec) - 1) >> level) & OSC_PVEC_MASK;
	//void *parent_clone = (void **)osc_region_getBytes(r, OSC_PVEC_NBRANCHES * sizeof(void *));
	...
	if(level == OSC_PVEC_NBITS){
		...
		if(child == NULL){
			P("no child");
			nodeToInsert = osc_pvec_newPath_m(r, level - OSC_PVEC_NBITS, tailnode);
		}else{
			P("have child");
			nodeToInsert = osc_pvec_pushTail_m(r, pvec, level - OSC_PVEC_NBITS, child, tailnode);
		}
	}
	osc_pvec_node_getArray(ret)[subidx] = nodeToInsert;//osc_pvec_node_copy(nodeToInsert);
	OSC_PVEC_RETURN_OBJ(ret);
...
t_osc_pvec_node *osc_pvec_newPath(t_osc_region r, int level, t_osc_pvec_node *node)
{
	...
	t_osc_pvec_node *ret = osc_pvec_node_alloc(r);
	osc_pvec_node_getArray(ret)[0] = osc_pvec_newPath(r, level - OSC_PVEC_NBITS, node);
	OSC_PVEC_RETURN_OBJ(ret);
}
...
t_osc_pvec_node *osc_pvec_newPath_m(t_osc_region r, int level, t_osc_pvec_node *node)
{
	...
	if(OSC_PVEC_GETRC(node) != 1){
		t_osc_pvec_node *n = osc_pvec_newPath(r, level, node);
		OSC_PVEC_RETURN_OBJ(n);
	}
	if(level == 0){
		OSC_PVEC_RETURN_OBJ(node);
	...
	t_osc_pvec_node *ret = osc_pvec_node_alloc(r);
	//t_osc_pvec_node *ret = node;
	osc_pvec_node_getArray(ret)[0] = osc_pvec_newPath_m(r, level - OSC_PVEC_NBITS, node);
	OSC_PVEC_RETURN_OBJ(ret);
}

// remove the last element
t_osc_pvec *osc_pvec_pop(t_osc_region r, t_osc_pvec *pvec)
{
	OSC_PVEC_ENTER
	int count = osc_pvec_length(pvec);
	int shift = osc_pvec_getShift(pvec);
	...
	if(count == 1){
		t_osc_pvec *pv = osc_pvec_alloc(r, freefn); // osc_pvec_empty
		OSC_PVEC_RETURN_OBJ(pv);
	}
	...
	t_osc_pvec_node *newroot = osc_pvec_popTail(r, pvec, shift, root);
	int newshift = shift;
	if(newroot == NULL){
		newroot = osc_pvec_node_alloc(r);//osc_pvec_node_empty;
	}
	if(shift > OSC_PVEC_NBITS && osc_pvec_node_getArray(newroot)[1] == NULL){
		newroot = (t_osc_pvec_node *)osc_pvec_node_getArray(newroot)[0];
		newshift -= OSC_PVEC_NBITS;
	}
	t_osc_pvec *pv = _osc_pvec_alloc(r, count - 1, newshift, newroot, taillength - 1, newTail, freefn);
	OSC_PVEC_RETURN_OBJ(pv);
}
...
t_osc_pvec_node *osc_pvec_popTail(t_osc_region r, t_osc_pvec *pvec, int level, t_osc_pvec_node *node)
{
	...
	if(level > OSC_PVEC_NBITS){
		...
		t_osc_pvec_node *newchild = osc_pvec_popTail(r, pvec, level - OSC_PVEC_NBITS, (t_osc_pvec_node *)osc_pvec_node_getArray(node)[subidx]);
...
t_osc_pvec2 *_osc_pvec2_alloc(t_osc_region r, t_osc_pvec *head, t_osc_pvec *tail)
{
	t_osc_pvec2 *pvec2 = osc_region_getBytes(r, sizeof(t_osc_pvec2));
	if(pvec2){
		pvec2->refcount = 1;
		pvec2->length = 0;
		pvec2->head = head;
...
t_osc_pvec2 *osc_pvec2_alloc(t_osc_region r, void (*freefn)(void *))
{
	OSC_PVEC_ENTER
	OSC_PVEC_RETURN_OBJ(_osc_pvec2_alloc(r, osc_pvec_alloc(r, freefn), osc_pvec_alloc(r, freefn)), osc_pvec_printRefCount);
}
...
t_osc_pvec2 *osc_pvec2_retain(t_osc_pvec2 *pvec2)
{
	return osc_pvec2_copy(pvec2);
}
...
t_osc_pvec2 *osc_pvec2_assocN(t_osc_region r, t_osc_pvec2 *pvec2, int i, void *val)
{
	if(!pvec2){
		return NULL;
	}
	...
	if(i <= headcount + tailcount){
		if(i < headcount){
			return _osc_pvec2_alloc(r, osc_pvec_assocN(r, head, headcount - i - 1, val), osc_pvec_copy(tail));
		}else{
			return _osc_pvec2_alloc(r, osc_pvec_copy(head), osc_pvec_assocN(r, tail, i - headcount, val));
		}
	}else{
		return pvec2;
	}
...
t_osc_pvec2 *osc_pvec2_assocN_m(t_osc_region r, t_osc_pvec2 *pvec2, int i, void *val)
{
	if(!pvec2){
		return NULL;
	}
	...
	if(i <= headcount + tailcount){
		if(i < headcount){
			osc_pvec_assocN_m(r, head, headcount - i - 1, val);
			return pvec2;
		}else{
			osc_pvec_assocN_m(r, tail, i - headcount, val);
			return pvec2;
		}
	}else{
		return pvec2;
...
t_osc_pvec2 *osc_pvec2_append(t_osc_region r, t_osc_pvec2 *pvec2, void *val)
{
	if(!pvec2){
		return NULL;
	}
	t_osc_pvec *head = pvec2->head;
	t_osc_pvec *tail = pvec2->tail;
	return _osc_pvec2_alloc(r, osc_pvec_copy(head), osc_pvec_assocN(r, tail, osc_pvec_length(tail), val));
}

t_osc_pvec2 *osc_pvec2_append_m(t_osc_region r, t_osc_pvec2 *pvec2, void *val)
{
	if(!pvec2){
		return NULL;
	}
	t_osc_pvec *tail = pvec2->tail;
	osc_pvec_assocN_m(r, tail, osc_pvec_length(tail), val);
	return pvec2;
}

t_osc_pvec2 *osc_pvec2_prepend(t_osc_region r, t_osc_pvec2 *pvec2, void *val)
{
	if(!pvec2){
		return NULL;
	}
	t_osc_pvec *head = pvec2->head;
	t_osc_pvec *tail = pvec2->tail;
	return _osc_pvec2_alloc(r, osc_pvec_assocN(r, head, osc_pvec_length(head), val), osc_pvec_copy(tail));
}

t_osc_pvec2 *osc_pvec2_prepend_m(t_osc_region r, t_osc_pvec2 *pvec2, void *val)
{
	if(!pvec2){
		return NULL;
	}
	t_osc_pvec *head = pvec2->head;
	osc_pvec_assocN_m(r, head, osc_pvec_length(head), val);
	return pvec2;
}

t_osc_pvec2 *osc_pvec2_popFirst(t_osc_region r, t_osc_pvec2 *pvec2)
{
	if(!pvec2){
		return NULL;
	}
...
t_osc_pvec2 *osc_pvec2_popLast(t_osc_region r, t_osc_pvec2 *pvec2)
{
	...
	t_osc_pvec *head = pvec2->head;
	t_osc_pvec *tail = pvec2->tail;
	return _osc_pvec2_alloc(r, osc_pvec_copy(head), osc_pvec_pop(r, tail));
}
...
{
	long n = 5;
	t_osc_pvec **pvec = osc_region_getBytes(r, n * sizeof(t_osc_pvec *));;
	t_osc_pvec *pvec_empty = osc_pvec_alloc(r, myfree);
	pvec[0] = osc_pvec_assocN(r, pvec_empty, 0, (void *)0);
	//t_osc_pvec_node *node = osc_pvec_nth(pvec[0], 0);
	for(long i = 1; i < n; i++){
		printf("**************************************************\n");
		pvec[i] = osc_pvec_assocN(r, pvec[i - 1], i, (void *)i);
	}
	printf("**************************************************\n");
	printf("HERE\n");
	//pvec[n - 1] = osc_pvec_assocN(r, pvec[n - 1], 0, (void *)10);
	t_osc_pvec *p = pvec[n - 1];
	pvec[n - 1] = osc_pvec_pop(r, p);
	printf("**************************************************\n");
	printf("FREE\n");
	osc_pvec_release(p);
	print_pvec(pvec[n - 1]);
